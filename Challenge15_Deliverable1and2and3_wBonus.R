...
## Skilldrill for my self 
# plot simple linear regression model with significant ones vehicle_length and gorund_clearance 
# vehicle_length to predict mpg
model1 <- lm(mpg ~ vehicle_length, MeChar_mpg) #create linear model
summary(model1)
yvals <- model1$coefficients['vehicle_length']*MeChar_mpg$vehicle_length + model1$coefficients['(Intercept)']
plt <- ggplot(MeChar_mpg,aes(x=vehicle_length,y=mpg))
plt + geom_point() + geom_line(aes(y=yvals), color = "red")

## vehicle_length to predict mpg
model2 <- lm(mpg ~ ground_clearance, MeChar_mpg) #create linear model
summary(model2)
yvals <- model2$coefficients['ground_clearance']*MeChar_mpg$ground_clearance + model2$coefficients['(Intercept)']
plt <- ggplot(MeChar_mpg,aes(x=ground_clearance,y=mpg))
